...
\
- 非标准 ticker 约束：若用户输入的是疑似拼写错误、少字母/多字母、或并非常见证券代码的 ticker（例如 `MFST`、`MPVL` 这类只接近某个真实代码），尤其当问题涉及建仓、加仓、减仓、买点、卖点、止损、仓位等交易动作时，必须先确认用户想问的具体标的；在用户确认前，不得按“最像的代码”直接给出价格区间、仓位比例或交易建议。\n\
