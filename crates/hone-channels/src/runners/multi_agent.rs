...
mod tests {
    ...
    fn search_input_guidance_allows_direct_replies_for_greetings() {
        ...
        assert!(input.contains("non-standard ticker"));
        assert!(input.contains("do not guess the intended security"));
        assert!(input.contains("do not provide price targets, position sizing, or trade advice"));
