...
describe("task-health-model", () => {
  ...
  it("formats run times and durations without page state", () => {
    ...
    expect(
      taskRunDurationMs("2026-05-13T00:00:00Z", "2026-05-13T00:00:03Z"),
    ).toBe(3000)
    expect(taskRunDurationMs("bad", "2026-05-13T00:00:03Z")).toBe(0)
    expect(
      taskRunDurationMs("2026-05-13T00:00:03Z", "2026-05-13T00:00:00Z"),
    ).toBe(0)
  })

  it("derives relative labels using an explicit clock", () => {
    setLocale("zh")
    const referenceNow = new Date("2026-05-13T00:00:00Z")

    expect(relativeTaskRunTime(null, referenceNow)).toBe("—")
    expect(relativeTaskRunTime("not-a-date", referenceNow)).toBe("not-a-date")
    expect(relativeTaskRunTime("2026-05-12T23:59:30Z", referenceNow)).toBe(
      "30 秒前",
    )
    expect(relativeTaskRunTime("2026-05-12T23:30:00Z", referenceNow)).toBe(
      "30 分钟前",
    )
    expect(relativeTaskRunTime("2026-05-12T21:00:00Z", referenceNow)).toBe(
      "3 小时前",
    )
    expect(relativeTaskRunTime("2026-05-11T00:00:00Z", referenceNow)).toBe(
      "2 天前",
    )
  })
  ...
  it("sorts summary rows and task filter options consistently", () => {
    const taskSummaryByName = {
      zeta: taskSummaryFixture({ runs_24h: 2 }),
      alpha: taskSummaryFixture({ runs_24h: 1 }),
    }

    expect(taskFilterOptions(taskSummaryByName)).toEqual(["alpha", "zeta"])
    expect(
      taskSummaryRows(taskSummaryByName).map((row) => [
        row.task,
        row.runs_24h,
      ]),
    ).toEqual([
