...
- 若需要"冷启动立即跑一次而不是等到第一个 tick",在 loop 前先 `work().await`,
  ...
  (参考 `spawn_event_source` 的 FixedInterval / CronAligned 分支)
...
- poller / EventSource: [`spawn_event_source`](../../crates/hone-event-engine/src/spawner.rs)
  的 `SourceSchedule::CronAligned` 分支
- internal task(digest scheduler、daily report): [`pipeline::cron_minute_tick`](../../crates/hone-event-engine/src/pipeline.rs)
...
它已经在 `spawn_event_source` 的 CronAligned 分支和 `pipeline::cron_minute_tick`
里实现,不要在 `engine.rs::start()` 内、或新模块里再抄。
...
- **内存 fired key**(防止同窗口重复触发):
  ...
  事实标准已经在 `pipeline::cron_minute_tick` / `spawner.rs` 内一致,
  新代码沿用
...
- [ ] interval / cron 哪个语义?如果是 cron-aligned,有没有走 `spawn_event_source` 或 `pipeline::cron_minute_tick`?
